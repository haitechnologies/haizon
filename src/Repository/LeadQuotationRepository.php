...
/**
 * LeadQuotation Repository
 *
 * Handles PDO-based data access for erp_quotations and erp_quotation_items
 * tables with strict tenant isolation. Same underlying tables as QuotationRepository
 * but scoped to lead-based quotations.
 */
class LeadQuotationRepository
{
    private Database $db;
    ...
    /**
     * Find lead quotation by ID and organization
     */
    public function find(int $id, int $orgId): ?LeadQuotation
    {
        ...
        return $this->mapRowToLeadQuotation($row);
    }

    /**
     * Find lead quotation items by quotation ID and organization
     */
    public function findItemsByQuotation(int $quotationId, int $orgId): array
    {
        ...
        $items = [];
        foreach ($rows as $row) {
            $items[] = $this->mapRowToLeadQuotationItem($row);
        }
        return $items;
    }
    ...
    /**
     * Find all lead quotations in an organization (scoped to lead_id > 0)
     */
    public function findAll(int $orgId): array
    {
        $sql = "SELECT * FROM `{DB::QUOTATIONS}` WHERE organization_id = :org_id AND lead_id > 0 ORDER BY id DESC";
        $rows = $this->db->fetchAll($sql, ['org_id' => $orgId]);
        $quotations = [];
        foreach ($rows as $row) {
            $quotations[] = $this->mapRowToLeadQuotation($row);
        }
        return $quotations;
    }

    /**
     * Save LeadQuotation (Insert or Update)
     */
    public function save(LeadQuotation $quotation): LeadQuotation
    {
    ...
    private function insert(LeadQuotation $quotation): LeadQuotation
    {
        ...
        if ($inserted === null) {
            throw new \RuntimeException("Failed to retrieve inserted lead quotation.");
        }

        return $inserted;
    }

    private function update(LeadQuotation $quotation): LeadQuotation
    {
        ...
        if ($updated === null) {
            throw new \RuntimeException("Failed to retrieve updated lead quotation.");
        }
    ...
    /**
     * Save LeadQuotationItem (Insert or Update)
     */
    public function saveItem(LeadQuotationItem $item): LeadQuotationItem
    {
        if ($item->id === null) {
    ...
    private function insertItem(LeadQuotationItem $item): LeadQuotationItem
    {
        ...
        if ($inserted === null) {
            throw new \RuntimeException("Failed to retrieve inserted lead quotation item.");
        }

        return $inserted;
    }

    private function updateItem(LeadQuotationItem $item): LeadQuotationItem
    {
        ...
        if ($updated === null) {
            throw new \RuntimeException("Failed to retrieve updated lead quotation item.");
        }
    ...
    /**
     * Find a single LeadQuotationItem by ID and organization
     */
    public function findItem(int $id, int $orgId): ?LeadQuotationItem
    {
        ...
        return $this->mapRowToLeadQuotationItem($row);
    }

    /**
     * Delete a lead quotation and its associated items
     */
    ...
    /**
     * Map database row to LeadQuotation DTO
     */
    private function mapRowToLeadQuotation(array $row): LeadQuotation
    {
    ...
    /**
     * Map database row to LeadQuotationItem DTO
     */
    private function mapRowToLeadQuotationItem(array $row): LeadQuotationItem
    {
        return new LeadQuotationItem(
            id: (int)$row['id'],
            organizationId: (int)$row['organization_id'],
