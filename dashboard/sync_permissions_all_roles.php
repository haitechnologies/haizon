...
try {
    ...
    // Fetch all roles
    $roles = [];
    $result = $conn->query("SELECT id FROM " . DB::ROLES);
    while ($row = $result->fetch_assoc()) {
        $roles[] = (int)$row['id'];
    }

    if (empty($roles)) {
        throw new Exception('No roles found in ' . DB::ROLES);
    }
    ...
    foreach ($roles as $roleId) {
        // Remove orphaned permissions for this role
        ...
        // Get allowed permission slugs for this role (pattern)
        $allowedSlugs = [];
        $sql = "
            SELECT DISTINCT mp.slug
        ...
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                $allowedSlugs[] = $row['slug'];
            }
        }

        if (empty($allowedSlugs)) {
            // Role has no existing permission pattern to follow.
            continue;
        }

        $allowedSlugList = "'" . implode("','", array_map([$conn, 'real_escape_string'], $allowedSlugs)) . "'";

        // Insert missing permissions that match role pattern
        $missingSql = "
            ...
            WHERE p.id IS NULL
              AND mp.slug IN ({$allowedSlugList})";
    ...
    echo "Sync completed.\n";
    echo "Deleted orphaned rows: {$totalDeleted}\n";
    echo "Inserted missing rows: {$totalInserted}\n";
} catch (Exception $e) {
    $conn->rollback();
