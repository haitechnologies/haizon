...
// Handle source purchase for creating debit note from purchase
$source_purchase_id = 0;
if (isset($_REQUEST['source_purchase_id']) && !empty($_REQUEST['source_purchase_id'])) {
    $source_purchase_id = e_s__($_REQUEST['source_purchase_id']);
}
...
if (($action == "update_$module" || $action == "add_$module") && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $debit_note_date             = e_s__($_POST['debit_note_date']);
    $vendor_id                = e_s__($_POST['vendor_id']);
    $debit_note_status          = !empty($_POST['debit_note_status']) ? e_s__($_POST['debit_note_status']) : 'draft';
    $reference_no               = e_s__($_POST['reference_no']);
    $purchase_id                 = isset($_POST['purchase_id']) ? e_s__($_POST['purchase_id']) : '0';
    $warehouse_id               = e_s__($_POST['warehouse_id']);
    $purchase_person               = e_s__($_POST['purchase_person']);
    ...
    $debit_note_date             = date('d-m-Y', time());
...
if ($action == "update_$module" && !empty($id)) {
    ...
    if (empty($vendor_id) || $vendor_id == 'Please select') {
        ...
        $debit_note_date        = processDateDtoY($debit_note_date);

        $purchase_person       = (empty($purchase_person) ? '0' : $purchase_person);
        $purchase_id         = (empty($purchase_id) || $purchase_id == '' ? '0' : $purchase_id);
        ...
        if ($update_row) {
            ...
            if ($total_rows > 0) {
                ...
                for ($debit_note_item = 1; $debit_note_item <= $total_rows; $debit_note_item++) {
                    ...
                    if (!empty($item_id) && !empty($item_service)) {
                        ...
                        $update_row = $mysqli->query("UPDATE `" . DB::DEBIT_NOTE_ITEMS . "` SET 
                        ...
                        fp__(DB::DEBIT_NOTE_ITEMS, $item_id);

                        // Process New Items
                    } else if (empty($item_id) && !empty($item_service)) {

                        $insert_row = $mysqli->query("INSERT INTO `" . DB::DEBIT_NOTE_ITEMS . "`(debit_note_id, service, description, qty, rate, sub_total, tax, tax_amount, total) VALUES ('" . $debit_note_id . "', '" . $item_service . "', '" . $item_description . "', '" . $item_qty . "', '" . $item_rate . "', '" . $item_sub_total . "', '" . $item_tax . "', '" . $item_tax_amount . "', '" . $item_total . "'); ");

                        if ($insert_row) $inserted_row++;
                        fp__(DB::DEBIT_NOTE_ITEMS, $mysqli->insert_id);

                        // Process Deleted Items
                    } else if (!empty($item_id) && empty($item_service) && empty($item_rate) && empty($item_tax) && empty($item_total)) {

                        $mysqli->query("DELETE FROM `" . DB::DEBIT_NOTE_ITEMS . "` WHERE id=$item_id");
                    }
                    // ---------------------------------------------
            ...
            if ($updated_row == 0 && $inserted_row == 0) {
                ...
                $error_message = "Please add at least one Debit Note Item.";
            } else {
...
*/
    ...
    if (empty($vendor_id) || $vendor_id == 'Please select') {
        ...
        $error_message = 'Please select Debit Note Date.';
    } else {
        ...
        if ($total_rows > 0) {
            ...
            for ($debit_note_item = 1; $debit_note_item <= $total_rows; $debit_note_item++) {
                ...
                if (!empty($item_service)) {
                    ...
                    if ($inserted_row == 0) {
                        ...
                        // ======================================================
                        // debit_note NO Auto Generation System
                        // ======================================================

                        // Build the prefix for this month
                        $prefix = 'FL-DN' . date('ym');

                        // Get the last debit_note number for this month
                        $sql = "SELECT debit_note_no  FROM `" . DB::DEBIT_NOTES . "`  WHERE debit_note_no LIKE '{$prefix}-%'ORDER BY debit_note_no DESC LIMIT 1";
                        ...
                        if ($row = $result->fetch_assoc()) {
                            ...
                            // First debit_note of the month
                            $new_serial = 1;
                        }

                        // Build new debit_note number with zero padding
            ...
            if ($inserted_row == 0) {
                ...
                if ($save_and_send == 1) {
                    header("Location:send_debit_note.php?id=$id");
                } else {
                    header("Location:listing_$module.php?success_message=$success_message");
                }
            }
...
|
*/
$created_by = getTableAttr('created_by', DB::DEBIT_NOTES, $id);
...
if (
    ...
    $purchase_id             = isset($row['purchase_id']) ? s__($row['purchase_id']) : '0';
    $warehouse_id           = s__($row['warehouse_id']);
    $purchase_person           = s__($row['purchase_person']);
    ...
    if ($total_rows > 0) {
        while ($row_debit_note_items = $result_debit_note_items->fetch_array()) {
            ...
            array_push($item_id_arr,                $row_debit_note_items['id']);
...
/*
|--------------------------------------------------------------------------
| POPULATE FROM SOURCE PURCHASE
|--------------------------------------------------------------------------
*/
if (!empty($source_purchase_id) && $action == 'add_debit_notes') {
    // Get purchase data
    $purchase_result = $mysqli->query("SELECT * FROM `" . DB::PURCHASES . "` WHERE id=$source_purchase_id");
    if ($purchase_result && $purchase_row = $purchase_result->fetch_array()) {
        
        $vendor_id                = s__($purchase_row['vendor_id']);
        $purchase_id                 = $source_purchase_id; // Link to source purchase
        $reference_no               = 'PO-' . s__($purchase_row['purchase_no']);
        $warehouse_id               = s__($purchase_row['warehouse_id']);
        $debit_note_status         = 'draft';
        $debit_note_date           = date('d-m-Y');
        $purchase_person               = '';
        
        $vendor_notes             = s__($purchase_row['vendor_notes']);
        $terms_and_conditions       = s__($purchase_row['terms_and_conditions']);
        
        $grand_subtotal             = s__($purchase_row['grand_subtotal']);
        $grand_discount_type        = s__($purchase_row['grand_discount_type']);
        $grand_discount_type_value  = s__($purchase_row['grand_discount_type_value']);
        $grand_discount_amount      = s__($purchase_row['grand_discount_amount']);
        $grand_after_discount       = s__($purchase_row['grand_after_discount']);
        $grand_tax                  = s__($purchase_row['grand_tax']);
        $grand_total                = s__($purchase_row['grand_total']);
        
        // Get purchase items
        $items_result = $mysqli->query("SELECT * FROM `" . DB::PURCHASE_ITEMS . "` WHERE purchase_id=$source_purchase_id ORDER BY id");
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                                <input type="text" class="form-control" placeholder="Requested Date" name="debit_note_date" id="debit_note_date" value="<?php echo $debit_note_date; ?>">
                                        ...
                                        <div class="col-lg-9">
                                            <select name="purchase_person" id="purchase_person" class="form-select">
                                                ...
                                                while ($rows = $result->fetch_array()) {
                                                    $warehouse_name = $rows["warehouse_name"];
                                                ?>
                                                    <option value="<?php echo $rows['id']; ?>" <?php if ($action == "edit_$module" && $rows['id'] == $purchase_person) { ?>selected <?php } else if ($rows['id'] == $purchase_person) { ?>selected <?php } ?>>
                                                        <?php echo $warehouse_name; ?>
                                                    </option>
                                                <?php } ?>
                            ...
                            <div class="row card-body">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    ?>
                                        ...
                                        <div class="mb-2">
                                            <div class="row mb-3 pb-3" id="row_<?php echo $debit_note_item; ?>">


                                                <div class="col-lg-12">
                                                    <div class="row">

                                                        <input type="hidden" name="item_id[]" id="item_id<?php echo $debit_note_item; ?>" value="<?php echo (!empty($item_id_arr[$index]) ? $item_id_arr[$index] : ''); ?>">
                                                        ...
                                                        <div class="col-lg-3">
                                                            <textarea name="description[]" id="description<?php echo $debit_note_item; ?>" rows="2" class="form-control" placeholder="Add a description to your item"><?php echo (!empty($description_arr[$index]) ? $description_arr[$index] : ''); ?></textarea>
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="qty[]" id="qty<?php echo $debit_note_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($qty_arr[$index]) ? $qty_arr[$index] : '1'); ?>" onkeyup="calculateItemAmount('<?php echo $debit_note_item; ?>');" onchange=" calculateItemAmount('<?php echo $debit_note_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="rate[]" id="rate<?php echo $debit_note_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($rate_arr[$index]) ? $rate_arr[$index] : '0'); ?>" onkeyup="calculateItemAmount('<?php echo $debit_note_item; ?>');" onchange=" calculateItemAmount('<?php echo $debit_note_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="sub_total[]" id="sub_total<?php echo $debit_note_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" value="<?php echo (!empty($sub_total_arr[$index]) ? $sub_total_arr[$index] : '0'); ?>"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <select name="tax[]" id="tax<?php echo $debit_note_item; ?>" class="form-select" onchange="calculateItemAmount(<?php echo $debit_note_item; ?>, this.value); ">
                                                                <?php
                                                                // -----------------------
                                                                for ($i = 0; $i <= 100; $i++) {
                                                                    // -----------------------
                                                            ...
                                                            <div class="text-center mt-1">
                                                                <span class="badge bg-light text-black" style="font-weight: normal;" id="div_tax_amount<?php echo $debit_note_item; ?>" style="display: <?php if (!empty($tax_arr[$index])) { ?> block <?php } else { ?> none <?php } ?>;">
                                                                    <span id="span_tax_amount<?php echo $debit_note_item; ?>">
                                                                        <?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>
                                                                    </span>
                                                                </span>
                                                            </div>

                                                            <input type="hidden" name="tax_amount[]" id="tax_amount<?php echo $debit_note_item; ?>" class="form-control" placeholder="0" value="<?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>">
                                                            <!-- <div class="form-text bg-light border border-top-0 rounded-bottom text-end px-2 py-1 mt-0">15,584</div> -->
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="total[]" id="total<?php echo $debit_note_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" placeholder="0" value="<?php echo (!empty($total_arr[$index]) ? $total_arr[$index] : ''); ?>" onchange="calculateGrand(<?php echo $debit_note_item; ?>);" onkeyup="calculateGrand(<?php echo $debit_note_item; ?>);"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-2 mt-1">
                                                            <?php if ($debit_note_item > 1) { ?>
                                                                <a href="#" onclick="calculateItemAmount('<?php echo $debit_note_item; ?>'); clear_row(<?php echo $debit_note_item; ?>)"><span class="badge bg-warning"> <i class="ph-x"></i> </span></a>
                                                            <?php } ?>
                                                        </div>
                                ...
                                <div class="">
                                    <span id="span_add_item_row<?php echo $debit_note_item; ?>"><a href="#" onclick="add_item_row(); "><span class="badge bg-primary"> Add New Row </a></span></span>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="ms-sm-3 mb-3 mb-sm-0">
                                        ...
                                        <textarea class="form-control" name="vendor_notes" id="vendor_notes" style="field-sizing: content;" placeholder="Enter any notes to be displayed in your transaction"><?php echo $vendor_notes; ?></textarea>
                                    </div>
                                </div>
                            </div>
