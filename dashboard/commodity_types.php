...
$module_caption     = 'Commodity Type';
$tbl_name = DB::COMMODITY_TYPES;
$error_message         = '';
$success_message     = '';
...
if ($action == "update_$module" && !empty($id) && granted('edit', $module_id)) {
    ...
    if (empty($commodity_type)) {
        $error_message = 'Commodity type is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'commodity_type', $commodity_type) && $commodity_type != getTableAttr('commodity_type', $tbl_name, $id)) {
        $error_message = 'Duplicate Commodity type. Please enter different.';
    } else {
...
*/
    ...
    if (empty($commodity_type)) {
        $error_message = 'Commodity type is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'commodity_type', $commodity_type)) {
        $error_message = 'Duplicate Commodity type. Please enter different.';
    } else {

        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(commodity_type, description, is_active) VALUES ('" . $commodity_type . "', '" . $description . "', '" . $publish . "'); ");
        ...
        <!-- Page header -->
                ...
                <div class="card col-lg-6">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <label class="col-lg-3 col-form-label"><span class="text-danger">Commodity Type:*</span></label>

                            <div class="col-lg-9">
                                <input required type="text" name="commodity_type" id="commodity_type" value="<?php echo $commodity_type; ?>" class="form-control">
                            </div>
                        </div>
