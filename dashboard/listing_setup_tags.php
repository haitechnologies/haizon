...
if (($action == "delete_$module" && !empty($id)) && granted('delete', $module_id)) {
    ...
    // INPUT VALIDATION: Validate tag ID
    $idResult = InputValidator::integer($id, 1);
    if (!$idResult['valid']) {
        $error_message = "Invalid tag ID: " . $idResult['error'];
    } else {
        $tagId = $idResult['value'];
        
        // IDOR PROTECTION: Check ownership (unless system admin)
        $canDelete = has_full_access();
        if (!$canDelete) {
            $canDelete = checkOwnership($tbl_name, $tagId, 'created_by');
        }
        ...
        if (!$canDelete) {
            ...
            if ($stmt->execute()) {
                ...
                log_error("Delete failed for tag $tagId: " . $stmt->error, 'ERROR', __FILE__, __LINE__);
            }
            $stmt->close();
        }
    }
