...
if (($action == "delete_$module" && !empty($id)) && granted('delete', $module_id)) {
    ...
    if (is_SuperAdmin()) {
        $mysqli->query("DELETE FROM `$tbl_name` WHERE id=$id AND context='sale'");
    } else {
        $mysqli->query("DELETE FROM `$tbl_name` WHERE id=$id AND context='sale' AND created_by ='" . $session_user_id . "'");
    }
...
$(document).ready(function() {
    window.HAIDatatableInitializer.init('#grid-<?php echo $module; ?>', '<?php echo $module; ?>', {
        ...
        language: { search: '', searchPlaceholder: 'Search sale types...', lengthMenu: '_MENU_' }
    });
});
</script>
