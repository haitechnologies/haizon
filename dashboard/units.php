...
if ($action == "update_$module" && !empty($id) && granted('edit', $module_id)) {
    ...
    } else if (checkDuplicateRow($tbl_name, 'unit', $unit) && $unit != getTableAttr('unit', $tbl_name, $id)) {
...
*/
    ...
    if (empty($unit)) {
        $error_message = 'Unit is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'unit', $unit)) {
        $error_message = 'Unit already exists. Please enter a different one.';
    } else {

        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(unit, publish) VALUES ('" . $unit . "', '" . $publish . "'); ");
