...
*/
if ($action == "update_$module" || $action == "add_$module") {
    $purchase_order_date        = e_s__($_POST['purchase_order_date']);
    // $expiry_date                = e_s__($_POST['expiry_date']);
    $vendor_id                  = e_s__($_POST['vendor_id']);
    $purchase_order_status      = e_s__($_POST['purchase_order_status']);
...
if ($action == "update_$module" && !empty($id)) {
    ...
    if (empty($vendor_id) || $vendor_id == 'Please select') {
    ...
    } else if (empty($purchase_order_date)) {
        $error_message = 'Please select Purchase order Date.';
        ...
        $purchase_order_date    = processDateDtoY($purchase_order_date);
        ...
        if ($update_row) {
            ...
            if ($total_rows > 0) {
                ...
                for ($purchase_order_item = 1; $purchase_order_item <= $total_rows; $purchase_order_item++) {
                    ...
                    if (!empty($item_id) && !empty($item_service)) {
                        ...
                        $update_row = $mysqli->query("UPDATE `" . DB::PURCHASE_ORDER_ITEMS . "` SET 
                        ...
                        fp__(DB::PURCHASE_ORDER_ITEMS, $item_id);

                        // Process New Items
                    } else if (empty($item_id) && !empty($item_service)) {

                        $insert_row = $mysqli->query("INSERT INTO `" . DB::PURCHASE_ORDER_ITEMS . "`(purchase_order_id, service, description, qty, rate, sub_total, tax, tax_amount, total) VALUES ('" . $purchase_order_id . "', '" . $item_service . "', '" . $item_description . "', '" . $item_qty . "', '" . $item_rate . "', '" . $item_sub_total . "', '" . $item_tax . "', '" . $item_tax_amount . "', '" . $item_total . "'); ");

                        if ($insert_row) $inserted_row++;
                        fp__(DB::PURCHASE_ORDER_ITEMS, $mysqli->insert_id);

                        // Process Deleted Items
                    } else if (!empty($item_id) && empty($item_service) && empty($item_rate) && empty($item_tax) && empty($item_total)) {

                        $mysqli->query("DELETE FROM `" . DB::PURCHASE_ORDER_ITEMS . "` WHERE id=$item_id");
                    }
                    // ---------------------------------------------
            ...
            if ($updated_row == 0 && $inserted_row == 0) {
                ...
                $error_message = "Please add at least one Purchase order Item.";
            } else {

                if ($save_and_send == 1) {
                    header("Location:send_purchase_order.php?id=$id");
                } else {
            ...
            for ($purchase_order_item = 1; $purchase_order_item <= $total_rows; $purchase_order_item++) {
                ...
                if (!empty($item_service)) {
                    ...
                    if ($inserted_row == 0) {
                        ...
                        // ======================================================
                        // Purchase_Order NO Auto Generation System
                        // ======================================================

                        // Build the prefix for this month
                        $prefix = 'FL-PO' . date('ym');

                        // Get the last purchase_order number for this month
                        $sql = "SELECT purchase_order_no  FROM `" . DB::PURCHASE_ORDERS . "`  WHERE purchase_order_no LIKE '{$prefix}-%'ORDER BY purchase_order_no DESC LIMIT 1";
                        ...
                        if ($row = $result->fetch_assoc()) {
                            ...
                            // First purchase_order of the month
                            $new_serial = 1;
                        }

                        // Build new purchase_order number with zero padding
...
|
*/
$created_by = getTableAttr('created_by', DB::PURCHASE_ORDERS, $id);
...
if (
    ...
    // ------------------ TOTAL ITEMS ------------------
    $result_purchase_order_items     = $mysqli->query("SELECT * FROM `" . DB::PURCHASE_ORDER_ITEMS . "` WHERE purchase_order_id=$id");
    ...
    if ($total_rows > 0) {
        while ($row_purchase_order_items = $result_purchase_order_items->fetch_array()) {
            ...
            array_push($item_id_arr,                $row_purchase_order_items['id']);
            ...
            // array_push($discount_type_arr,          $row_purchase_order_items['discount_type']);
            // array_push($discount_type_value_arr,    $row_purchase_order_items['discount_type_value']);
            // array_push($discount_amount_arr,        $row_purchase_order_items['discount_amount']);
                        ...
                        <div class="col-lg-6">
                            <div class="card">
                                ...
                                <div class="card-header d-flex align-items-center">
                                    <h6 class="mb-0">
                                        <?php if (($action == "edit_$module" || $action == "update_$module") && !empty($id)) { ?>Update<?php } else { ?>New<?php } ?> Purchase Order
                                    </h6>
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label"><span class="text-danger">Purchase Order Date:*</span></label>
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                                <input type="text" class="form-control" placeholder="Requested Date" name="purchase_order_date" id="purchase_order_date" value="<?php echo $purchase_order_date; ?>">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <input type="text" class="form-control" placeholder="Let your vendor know what this Purchase order is for" name="subject" id="subject" value="<?php echo $subject; ?>">
                                        </div>
                                    </div>
                            ...
                            <div class="row card-body">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    ?>
                                        ...
                                        <div class="mb-2">
                                            <div class="row mb-3 pb-3" id="row_<?php echo $purchase_order_item; ?>">


                                                <div class="col-lg-12">
                                                    <div class="row">

                                                        <input type="hidden" name="item_id[]" id="item_id<?php echo $purchase_order_item; ?>" value="<?php echo (!empty($item_id_arr[$index]) ? $item_id_arr[$index] : ''); ?>">
                                                        ...
                                                        <div class="col-lg-3">
                                                            <textarea name="description[]" id="description<?php echo $purchase_order_item; ?>" rows="2" class="form-control" placeholder="Add a description to your item"><?php echo (!empty($description_arr[$index]) ? $description_arr[$index] : ''); ?></textarea>
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="qty[]" id="qty<?php echo $purchase_order_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($qty_arr[$index]) ? $qty_arr[$index] : '0'); ?>" onkeyup="calculateItemAmount('<?php echo $purchase_order_item; ?>');" onchange=" calculateItemAmount('<?php echo $purchase_order_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="rate[]" id="rate<?php echo $purchase_order_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($rate_arr[$index]) ? $rate_arr[$index] : '0'); ?>" onkeyup="calculateItemAmount('<?php echo $purchase_order_item; ?>');" onchange=" calculateItemAmount('<?php echo $purchase_order_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="sub_total[]" id="sub_total<?php echo $purchase_order_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" value="<?php echo (!empty($sub_total_arr[$index]) ? $sub_total_arr[$index] : '0'); ?>"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <select name="tax[]" id="tax<?php echo $purchase_order_item; ?>" class="form-select" onchange="calculateItemAmount(<?php echo $purchase_order_item; ?>, this.value); ">
                                                                <?php
                                                                // -----------------------
                                                                for ($i = 0; $i <= 100; $i++) {
                                                                    // -----------------------
                                                            ...
                                                            <div class="text-center mt-1">
                                                                <span class="badge bg-light text-black" style="font-weight: normal;" id="div_tax_amount<?php echo $purchase_order_item; ?>" style="display: <?php if (!empty($tax_arr[$index])) { ?> block <?php } else { ?> none <?php } ?>;">
                                                                    <span id="span_tax_amount<?php echo $purchase_order_item; ?>">
                                                                        <?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>
                                                                    </span>
                                                                </span>
                                                            </div>

                                                            <input type="hidden" name="tax_amount[]" id="tax_amount<?php echo $purchase_order_item; ?>" class="form-control" placeholder="0" value="<?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>">
                                                            <!-- <div class="form-text bg-light border border-top-0 rounded-bottom text-end px-2 py-1 mt-0">15,584</div> -->
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="total[]" id="total<?php echo $purchase_order_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" placeholder="0" value="<?php echo (!empty($total_arr[$index]) ? $total_arr[$index] : ''); ?>" onchange="calculateGrand(<?php echo $purchase_order_item; ?>);" onkeyup="calculateGrand(<?php echo $purchase_order_item; ?>);"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-2 mt-1">
                                                            <?php if ($purchase_order_item > 1) { ?>
                                                                <a href="#" onclick="calculateItemAmount('<?php echo $purchase_order_item; ?>'); clear_row(<?php echo $purchase_order_item; ?>)"><span class="badge bg-warning"> <i class="ph-x"></i> </span></a>
                                                            <?php } ?>
                                                        </div>
                                ...
                                <div class="">
                                    <span id="span_add_item_row<?php echo $purchase_order_item; ?>"><a href="#" onclick="add_item_row(); "><span class="badge bg-primary"> Add New Row </a></span></span>
                                </div>
