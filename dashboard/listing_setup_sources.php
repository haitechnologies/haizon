...
$tbl_name = DB::TAXONOMIES;  // Setup sources table
$error_message = '';
$success_message = '';
...
if (($action == "delete_$module" && !empty($id)) && granted('delete', $module_id)) {
    ...
    // INPUT VALIDATION: Validate source ID
    $idResult = InputValidator::integer($id, 1);
    if (!$idResult['valid']) {
        $error_message = "Invalid source ID: " . $idResult['error'];
    } else {
        $sourceId = $idResult['value'];
        
        // IDOR PROTECTION: Check ownership (unless system admin)
        $canDelete = has_full_access();
        if (!$canDelete) {
            $canDelete = checkOwnership($tbl_name, $sourceId, 'created_by');
        }
        
        if (!$canDelete) {
            $error_message = "You do not have permission to delete this source";
            log_error("IDOR attempt: User $session_user_id tried to delete source $sourceId", 'WARNING', __FILE__, __LINE__);
        } else {
            // Perform delete with prepared statement
            $stmt = $mysqli->prepare("DELETE FROM `" . $tbl_name . "` WHERE id=? AND type IN ('customer_source', 'lead_source')");
            ...
            if ($stmt->execute()) {
                ...
                log_error("Delete failed for source $sourceId: " . $stmt->error, 'ERROR', __FILE__, __LINE__);
            }
            $stmt->close();
        }
    }
