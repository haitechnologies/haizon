...
// $rs_temp     = $mysqli->query("SELECT id FROM `" . DB::QUOTATIONS . "` WHERE temporary=1 AND created_by=$session_user_id");
// $temporary   = $rs_temp->num_rows;

// echo $id;

if (empty($id)) {

    // DELETE OLD TEMPORARY TABLE
    $mysqli->query("DELETE FROM `" . DB::QUOTATIONS . "` WHERE temporary=1 AND created_by=$session_user_id");
    ...
    // IN CASE THIS IS REDIRECTED FROM THE CUSTOMERS OR LEADS MODULE
    $customer_id    = ((isset($_REQUEST['customer_id']) && !empty($_REQUEST['customer_id'])) ? e_s__($_REQUEST['customer_id']) : '0');
    ...
    $sql_customer_id = (empty($customer_id) || $customer_id === '0') ? "NULL" : "'" . $customer_id . "'";
    $sql_lead_id     = (empty($lead_id) || $lead_id === '0') ? "NULL" : "'" . $lead_id . "'";

    $mysqli->query("INSERT INTO `$tbl_name`(quotation_no, temporary, quotation_date, expiry_date, customer_id, lead_id) VALUES ('" . $quotation_no . "', 1, '" . date('Y-m-d', time()) . "', '" . date('Y-m-d', time()) . "', " . $sql_customer_id . ", " . $sql_lead_id . "); ");

    $id = $mysqli->insert_id;
    fp__($tbl_name, $id);

    $action = 'update_quotations';
}
...
// IN CASE THIS IS REDIRECTED FROM THE CUSTOMERS MODULE
// $customer_id    = ((isset($_REQUEST['customer_id']) && !empty($_REQUEST['customer_id'])) ? e_s__($_REQUEST['customer_id']) : '');
...
} else if (isset($_REQUEST['customer_id']) && !empty($_REQUEST['customer_id'])) {
    $customer_id     = e_s__($_REQUEST['customer_id']);
}

if (isset($_POST['lead_id']) && !empty($_POST['lead_id'])) {
    $lead_id     = e_s__($_POST['lead_id']);
} else if (isset($_REQUEST['lead_id']) && !empty($_REQUEST['lead_id'])) {
    $lead_id     = e_s__($_REQUEST['lead_id']);
}

if (!empty($lead_id) && $lead_id !== '0') {
    $customer_id = 0;
} else if (!empty($customer_id) && $customer_id !== '0' && $customer_id !== 'Please select') {
    $lead_id = 0;
}
...
$total_dim_rows         = 1;
if (isset($_POST['total_dim_rows']) && !empty($_POST['total_dim_rows'])) {
    $total_dim_rows            = e_s__($_POST['total_dim_rows']);
}
...
$quotation_date             = ((isset($_POST['quotation_date']) && !empty($_POST['quotation_date'])) ? e_s__($_POST['quotation_date']) : '');
...
if ($action == "update_$module" || $action == "add_$module") {
    ...
    $quotation_status           = ((isset($_POST['quotation_status']) && !empty($_POST['quotation_status'])) ? e_s__($_POST['quotation_status']) : '');
...
if ($action == "update_$module" && !empty($id)) {
    ...
    if ((empty($customer_id) || $customer_id == 'Please select' || $customer_id == '0') && (empty($lead_id) || $lead_id == 'Please select' || $lead_id == '0')) {
        $error_message = 'Please select Customer or Lead.';
    } else if (empty($job_reference_no)) {
        $error_message = 'Job Reference no is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'job_reference_no', $job_reference_no) && $job_reference_no != getTableAttr('job_reference_no', $tbl_name, $id)) {
        $error_message = 'Duplicate Job Reference No already exist. Please enter different.';
    } else if (empty($quotation_date)) {
        $error_message = 'Please select Quotation Date.';
    } else {

        if (empty($quotation_status)) {
            $quotation_status = getTableAttr('quotation_status', $tbl_name, $id);
        }
        ...
        // UPDATE 
        // ---------------------------------------------
        $sql_customer_id  = (empty($customer_id) || $customer_id == 'Please select' || $customer_id == '0') ? "NULL" : "'" . $customer_id . "'";
        $sql_lead_id      = (empty($lead_id) || $lead_id == 'Please select' || $lead_id == '0') ? "NULL" : "'" . $lead_id . "'";
        $sql_warehouse_id = (empty($warehouse_id) || $warehouse_id == 'Please select' || $warehouse_id == '0') ? "NULL" : "'" . $warehouse_id . "'";
        $sql_shipper_id   = (empty($shipper_id) || $shipper_id == 'Please select' || $shipper_id == '0') ? "NULL" : "'" . $shipper_id . "'";
        $sql_consignee_id = (empty($consignee_id) || $consignee_id == 'Please select' || $consignee_id == '0') ? "NULL" : "'" . $consignee_id . "'";
        ...
        $update_row = $mysqli->query("
                                        UPDATE `$tbl_name` SET
                                            ...
                                            mawb_bol		            = '" . $mawb_bol . "',
                                            hwb_hbol		            = '" . $hwb_hbol . "',
                                            shipper_id		            = " . $sql_shipper_id . ",
                                            consignee_id		        = " . $sql_consignee_id . ",
        ...
        if ($update_row) {
            ...
            if ($updated_row == 0 && $inserted_row == 0) {
                ...
                if ($save_and_send == 1) {
                    header("Location:send_quotation.php?id=$id");
                } else {
...
*/
    ...
    if ((empty($customer_id) || $customer_id == 'Please select' || $customer_id == '0') && (empty($lead_id) || $lead_id == 'Please select' || $lead_id == '0')) {
        ...
        $error_message = 'Job Reference no is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'job_reference_no', $job_reference_no)) {
        $error_message = 'Job Reference no already exists. Please enter a different one.';
    } else {
            ...
            for ($quotation_item = 1; $quotation_item <= $total_rows; $quotation_item++) {
                ...
                if (!empty($item_service)) {
                    ...
                    if ($inserted_row == 0) {
                        ...
                        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(quotation_no, customer_id, lead_id, quotation_status, quotation_date, expiry_date, job_reference_no, warehouse_id, expected_shipment_date, payment_term, shipment_type, sales_person, mawb_bol, hwb_hbol, shipper_id, consignee_id, origin_port, origin_country, destination_port, destination_country, gross_weight, volume, chargeable_weight, cbm, terms_and_conditions, grand_subtotal, grand_discount_type, grand_discount_type_value, grand_discount_amount, grand_after_discount, customer_notes, grand_tax, grand_total, is_active) VALUES ('" . $quotation_no . "', " . $sql_customer_id . ", " . $sql_lead_id . ", '" . $quotation_status . "',  '" . $quotation_date . "', '" . $expiry_date . "', '" . $job_reference_no . "', " . $sql_warehouse_id . ", '" . $expected_shipment_date . "', '" . $payment_term . "', '" . $shipment_type . "', '" . $sales_person . "',  '" . $mawb_bol . "',  '" . $hwb_hbol . "', " . $sql_shipper_id . ", " . $sql_consignee_id . ", '" . $origin_port . "', '" . $origin_country . "', '" . $destination_port . "', '" . $destination_country . "', '" . $gross_weight . "', '" . $volume . "', '" . $chargeable_weight . "', '" . $cbm . "', '" . $terms_and_conditions . "',   '" . $grand_subtotal . "',  '" . $grand_discount_type . "',  '" . $grand_discount_type_value . "',  '" . $grand_discount_amount . "',  '" . $grand_after_discount . "',   '" . $customer_notes . "',  '" . $grand_tax . "', '" . $grand_total . "', '" . $publish . "'); ");
...
if (
    ...
    // Preserve Job Reference No on validation errors / refresh after POST
    if (!empty($_POST['job_reference_no'])) {
        $job_reference_no = e_s__($_POST['job_reference_no']);
    }

    // Preserve Customer on validation errors / refresh after POST
    if (!empty($_POST['customer_id']) && $_POST['customer_id'] !== 'Please select') {
        $customer_id = e_s__($_POST['customer_id']);
    }

    // Preserve Lead on validation errors / refresh after POST
    if (!empty($_POST['lead_id']) && $_POST['lead_id'] !== 'Please select') {
        $lead_id = e_s__($_POST['lead_id']);
    }

    // Preserve Volume, Chargeable Weight, and CBM on validation errors / refresh after POST
    ...
    $result_dim_items       = $mysqli->query("SELECT * FROM `" . DB::DIMENSION_ITEMS . "` WHERE module_type='quotations' AND record_id=$id ORDER BY id ASC");
        ...
        <div class="page-header-content border-top py-2 px-3 carriers-page-header-content">
            <div class="my-1 d-flex align-items-center gap-2">
                ...
                <?php if (($action == "edit_$module" || $action == "update_$module") && !empty($id)) { ?>
                    <span class="text-muted small">(Quotation #: <?php echo $quotation_no; ?>)</span>
                <?php } ?>
                <span class="text-muted small"><strong><?php echo ((!empty($quotation_status)) ? ucwords($quotation_status) : ''); ?></strong></span>
            </div>

            <div class="my-1">
                <?php if (isset($module_id) && granted('create', $module_id)) { ?>
                    <?php if (!empty($id)) { ?>
                        <button type="button" onclick="document.getElementById('frmquotations').submit();" class="btn btn-primary btn-sm me-2">Save</button>
                    <?php } else { ?>
                        <button type="button" onclick="document.getElementById('quotation_status').value='draft'; document.getElementById('frmquotations').submit();" class="btn btn-primary btn-sm me-2">Save as Draft</button>
                    <?php } ?>

                    <button type="button" onclick="document.getElementById('save_and_send').value='1';<?php echo (!empty($id) ? '' : " document.getElementById('quotation_status').value='draft';"); ?> document.getElementById('frmquotations').submit();" class="btn btn-info btn-sm me-2">Save and Send</button>
                <?php } ?>
    ...
    <div class="content-inner">
        <div class="content">
            ...
            <form class="steps-basic clearfix" method="post" id="frm<?php echo $module; ?>" name="frm<?php echo $module; ?>" action="<?php echo $module; ?>.php" enctype="multipart/form-data">
                <input type="hidden" name="quotation_status" id="quotation_status" value="" />
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <select name="customer_id" id="customer_id" class="form-control select" onchange="toggleQuotationPartySelectors();">
                                    ...
                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label"><span class="text-danger">Lead Name:*</span></label>
                                        <div class="col-lg-9">
                                            <select name="lead_id" id="lead_id" class="form-control select" onchange="toggleQuotationPartySelectors();">
                                                <option value='0'>Please select</option>
                                                <?php
                                                // -------------------------------------------------------------------------------------------------
                                                ...
                                                while ($rows_leads = $result_leads->fetch_array()) {
                                                    // -------------------------------------------------------------------------------------------------
                                                ?>
                                                    <option value="<?php echo $rows_leads['id']; ?>" <?php if ($action == "edit_$module" && $rows_leads['id'] == $lead_id) { ?>selected <?php } else if ($rows_leads['id'] == $lead_id) { ?>selected <?php } ?>>
                                                        <?php echo $rows_leads["display_name"]; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                            <small class="text-muted">Select either Customer or Lead.</small>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                                <input type="text" class="form-control" name="quotation_date" id="quotation_date" value="<?php echo $quotation_date; ?>">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                                <input type="text" class="form-control" name="expected_shipment_date" id="expected_shipment_date" value="<?php echo $expected_shipment_date; ?>">
                                                <div class="form-control-feedback-icon">
                                                    <i class="ph-calendar"></i>
                                                </div>
                                            </div>
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <select class="form-select" name="shipment_type" id="shipment_type">
                                                ...
                                                <option value="transit_in" <?php if ($action == "edit_$module" && $shipment_type == 'transit_in') { ?>selected <?php } else if ($shipment_type == 'transit_in') { ?>selected <?php } ?>>Transit-In</option>

                                                <option value="transit_out" <?php if ($action == "edit_$module" && $shipment_type == 'transit_out') { ?>selected <?php } else if ($shipment_type == 'transit_out') { ?>selected <?php } ?>>Transit-Out</option>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-4">
                                            <select class="form-select <?php echo ((!empty($destination_port)) ? 'bg-light' : '') ?>" name="origin_country" id="origin_country" <?php echo ((!empty($destination_port)) ? 'style="pointer-events:none;"' : '') ?> onchange="ajax_select_country_ports('origin');"> <!--  style="pointer-events:none;"-->
                                                <option value="0"></option>
                                                <?php
                                                // -------------------------------------------------------------------------------------------------
                                                $result = $mysqli->query("SELECT * FROM `" . $tbl_prefix . "geo_countries` WHERE is_active=1 ORDER BY country");
                                    ...
                                    <div class="row border-top-black border-top-lg mt-3">
                                                ...
                                                <div class="modal-content">
                                                    <form id="popupForm">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mt-4">
                                                                <span id="span_add_dim_item_row<?php echo $quotation_item; ?>"><a href="#" onclick="add_dim_item_row(); "><span class="badge bg-primary"> Add New Dimension </a></span></span>
                                                            </div>
                                        ...
                                        <script>
                                            function add_dim_item_row() {
                                                var div_add_dim_here = document.getElementById('div_add_dim_here');
                                        ...
                                        <div class="row mb-2 mt-3">
                                            ...
                                            <div class="row mb-2" style=" <?php if ($total_dim_rows == 0) {
                                                                                echo 'display:none';
                                                                            } ?>">
                                                ...
                                                ?>
                                                    <div class="col-lg-12 mt-2">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-lg-1">
                                                                <label class="form-label"><?php echo (!empty($dim_pcs_arr[$dim_index]) ? $dim_pcs_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-1">
                                                                <label class="form-label"><?php echo (!empty($dim_unit_arr[$dim_index]) ? $dim_unit_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-2">
                                                                <label class="form-label"><?php echo (!empty($dim_length_arr[$dim_index]) ? $dim_length_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-2">
                                                                <label class="form-label"><?php echo (!empty($dim_width_arr[$dim_index]) ? $dim_width_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-2">
                                                                <label class="form-label"><?php echo (!empty($dim_height_arr[$dim_index]) ? $dim_height_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-2">
                                                                <label class="form-label"><?php echo (!empty($dim_cbm_arr[$dim_index]) ? $dim_cbm_arr[$dim_index] : ''); ?></label>
                                                            </div>

                                                            <div class="col-lg-2">
                                                                <label class="form-label"><?php echo (!empty($dim_volume_arr[$dim_index]) ? $dim_volume_arr[$dim_index] : ''); ?></label>
                                                            </div>
                                ...
                                <div class="row card-body">
                                                ...
                                                <div class="row mb-3 pb-3" id="row_<?php echo $quotation_item; ?>">
                                                    ...
                                                    <div class="col-lg-12">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-lg-1">
                                                                <input type="number" step="1" name="qty[]" id="qty<?php echo $quotation_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($qty_arr[$index]) ? $qty_arr[$index] : '1'); ?>" onkeyup="calculateItemAmount('<?php echo $quotation_item; ?>');" onchange=" calculateItemAmount('<?php echo $quotation_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                            </div>
                                    ...
                                    <script>
                                        ...
                                        function clear_row(row_no) {
                                            ...
                                            document.getElementById('service' + row_no).text = '';
