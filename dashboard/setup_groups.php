...
if ($action == "update_$module" && !empty($id) && granted('edit', $module_id)) {
    ...
    } else if (checkDuplicateRow($tbl_name, 'group_name', $group_name) && $group_name != getTableAttr('group_name', $tbl_name, $id)) {
...
*/
    ...
    if (empty($group_name)) {
        $error_message = 'Group Name is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'group_name', $group_name)) {
        $error_message = 'Group Name already exists. Please enter a different one.';
    } else {

        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(group_name, description, publish) VALUES ('" . $group_name . "', '" . $description . "', '" . $publish . "'); ");
