...
$r = $mysqli->query("SELECT t.id, t.purchase_status, t.purchase_date, t.grand_total, t.customer_id FROM `" . DB::PURCHASES . "` t WHERE t.id > 0 AND t.customer_id != '' $where ORDER BY t.id DESC LIMIT 25");
$c = $mysqli->query("SELECT COUNT(*) FROM `" . DB::PURCHASES . "` t WHERE t.id > 0 AND t.customer_id != '' $where")->fetch_row();
$total = $c[0] ?? 0;
