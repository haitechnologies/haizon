...
if ($action == "update_$module" && !empty($id) && granted('edit', $module_id)) {
    ...
    } else if (checkDuplicateRow($tbl_name, 'purchase_type', $purchase_type) && $purchase_type != getTableAttr('purchase_type', $tbl_name, $id)) {
...
*/
    ...
    if (empty($purchase_type)) {
        $error_message = 'Purchase type is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'purchase_type', $purchase_type)) {
        $error_message = 'Purchase type already exists. Please enter a different one.';
    } else {

        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(purchase_type, description, publish) VALUES ('" . $purchase_type . "', '" . $description . "', '" . $publish . "'); ");
...
if (!empty($id)) {
    ...
    $purchase_type          = s__($row['purchase_type']);
