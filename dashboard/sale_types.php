...
if ($action == "update_$module" && !empty($id) && granted('edit', $module_id)) {
    ...
    } else if (checkDuplicateRow($tbl_name, 'sale_type', $sale_type) && $sale_type != getTableAttr('sale_type', $tbl_name, $id)) {
...
*/
    ...
    if (empty($sale_type)) {
        $error_message = 'Sale type is mandatory.';
    } else if (checkDuplicateRow($tbl_name, 'sale_type', $sale_type)) {
        $error_message = 'Sale type already exists. Please enter a different one.';
    } else {

        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(sale_type, description, publish) VALUES ('" . $sale_type . "', '" . $description . "', '" . $publish . "'); ");
