...
// Handle source invoice for creating credit note from invoice
$source_invoice_id = 0;
if (isset($_REQUEST['source_invoice_id']) && !empty($_REQUEST['source_invoice_id'])) {
    $source_invoice_id = e_s__($_REQUEST['source_invoice_id']);
}
...
if (($action == "update_$module" || $action == "add_$module") && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $credit_note_date             = e_s__($_POST['credit_note_date']);
    $customer_id                = e_s__($_POST['customer_id']);
    $credit_note_status          = !empty($_POST['credit_note_status']) ? e_s__($_POST['credit_note_status']) : 'draft';
    $reference_no               = e_s__($_POST['reference_no']);
    $invoice_id                 = isset($_POST['invoice_id']) ? e_s__($_POST['invoice_id']) : '0';
    ...
    $credit_note_date             = date('d-m-Y', time());
...
if ($action == "update_$module" && !empty($id)) {
    ...
    if (empty($customer_id) || $customer_id == 'Please select') {
    ...
    } else if (empty($credit_note_date)) {
    } else {
        ...
        $credit_note_date        = processDateDtoY($credit_note_date);

        $sales_person       = (empty($sales_person) ? '0' : $sales_person);
        $invoice_id         = (empty($invoice_id) || $invoice_id == '' ? '0' : $invoice_id);
        ...
        if ($update_row) {
            ...
            if ($total_rows > 0) {
                ...
                for ($credit_note_item = 1; $credit_note_item <= $total_rows; $credit_note_item++) {
                    ...
                    if (!empty($item_id) && !empty($item_service)) {
                        ...
                        $update_row = $mysqli->query("UPDATE `" . DB::CREDIT_NOTE_ITEMS . "` SET 
                        ...
                        fp__(DB::CREDIT_NOTE_ITEMS, $item_id);

                        // Process New Items
                    } else if (empty($item_id) && !empty($item_service)) {

                        $insert_row = $mysqli->query("INSERT INTO `" . DB::CREDIT_NOTE_ITEMS . "`(credit_note_id, service, description, qty, rate, sub_total, tax, tax_amount, total) VALUES ('" . $credit_note_id . "', '" . $item_service . "', '" . $item_description . "', '" . $item_qty . "', '" . $item_rate . "', '" . $item_sub_total . "', '" . $item_tax . "', '" . $item_tax_amount . "', '" . $item_total . "'); ");

                        if ($insert_row) $inserted_row++;
                        fp__(DB::CREDIT_NOTE_ITEMS, $mysqli->insert_id);

                        // Process Deleted Items
                    } else if (!empty($item_id) && empty($item_service) && empty($item_rate) && empty($item_tax) && empty($item_total)) {

                        $mysqli->query("DELETE FROM `" . DB::CREDIT_NOTE_ITEMS . "` WHERE id=$item_id");
                    }
                    // ---------------------------------------------
            ...
            if ($updated_row == 0 && $inserted_row == 0) {
                ...
                $error_message = "Please add at least one Credit Note Item.";
            } else {
...
*/
    ...
    if (empty($customer_id) || $customer_id == 'Please select') {
        ...
        $error_message = 'Please select Credit Note Date.';
    } else {
        ...
        if ($total_rows > 0) {
            ...
            for ($credit_note_item = 1; $credit_note_item <= $total_rows; $credit_note_item++) {
                ...
                if (!empty($item_service)) {
                    ...
                    if ($inserted_row == 0) {
                        ...
                        // ======================================================
                        // credit_note NO Auto Generation System
                        // ======================================================
                        ...
                        $sql = "SELECT credit_note_no  FROM `" . DB::CREDIT_NOTES . "`  WHERE credit_note_no LIKE '{$prefix}-%'ORDER BY credit_note_no DESC LIMIT 1";
                        ...
                        if ($row = $result->fetch_assoc()) {
                            ...
                            // First credit_note of the month
                            $new_serial = 1;
                        }

                        // Build new credit_note number with zero padding
                        $credit_note_no = $prefix . '-' . str_pad($new_serial, 4, '0', STR_PAD_LEFT);
                        // ======================================================


                        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(credit_note_no, customer_id, credit_note_status, credit_note_date, reference_no, invoice_id, warehouse_id, sales_person, terms_and_conditions, grand_subtotal, grand_discount_type, grand_discount_type_value, grand_discount_amount, grand_after_discount, customer_notes, grand_tax, grand_total, publish) VALUES ('" . $credit_note_no . "', '" . $customer_id . "', '" . $credit_note_status . "',  '" . $credit_note_date . "', '" . $reference_no . "', '" . $invoice_id . "', '" . $warehouse_id . "', '" . $sales_person . "', '" . $terms_and_conditions . "',   '" . $grand_subtotal . "',  '" . $grand_discount_type . "',  '" . $grand_discount_type_value . "',  '" . $grand_discount_amount . "',  '" . $grand_after_discount . "',   '" . $customer_notes . "',  '" . $grand_tax . "', '" . $grand_total . "', '" . $publish . "'); ");
            ...
            if ($inserted_row == 0) {
                ...
                if ($save_and_send == 1) {
                    header("Location:send_credit_note.php?id=$id");
                } else {
                    header("Location:listing_$module.php?success_message=$success_message");
                }
            }
...
|
*/
$created_by = getTableAttr('created_by', DB::CREDIT_NOTES, $id);
...
if (
    ...
    $invoice_id             = isset($row['invoice_id']) ? s__($row['invoice_id']) : '0';
    ...
    // ------------------ TOTAL ITEMS ------------------
    $result_credit_note_items     = $mysqli->query("SELECT * FROM `" . DB::CREDIT_NOTE_ITEMS . "` WHERE credit_note_id=$id");
    ...
    if ($total_rows > 0) {
        while ($row_credit_note_items = $result_credit_note_items->fetch_array()) {
            ...
            array_push($item_id_arr,                $row_credit_note_items['id']);
            ...
            // array_push($discount_type_arr,          $row_credit_note_items['discount_type']);
            // array_push($discount_type_value_arr,    $row_credit_note_items['discount_type_value']);
            // array_push($discount_amount_arr,        $row_credit_note_items['discount_amount']);
...
if (!empty($source_invoice_id) && $action == 'add_credit_notes') {
    ...
    if ($invoice_result && $invoice_row = $invoice_result->fetch_array()) {
        ...
        $invoice_id                 = $source_invoice_id; // Link to source invoice
        $reference_no               = 'INV-' . s__($invoice_row['invoice_no']);
        $warehouse_id               = s__($invoice_row['warehouse_id']);
        $credit_note_status         = 'draft';
        $credit_note_date           = date('d-m-Y');
        $sales_person               = '';
        
        $customer_notes             = s__($invoice_row['customer_notes']);
        $terms_and_conditions       = s__($invoice_row['terms_and_conditions']);
        
        $grand_subtotal             = s__($invoice_row['grand_subtotal']);
        $grand_discount_type        = s__($invoice_row['grand_discount_type']);
        $grand_discount_type_value  = s__($invoice_row['grand_discount_type_value']);
        $grand_discount_amount      = s__($invoice_row['grand_discount_amount']);
        $grand_after_discount       = s__($invoice_row['grand_after_discount']);
        $grand_tax                  = s__($invoice_row['grand_tax']);
        ...
        if ($total_rows > 0) {
            ...
            while ($item_row = $items_result->fetch_array()) {
                array_push($item_id_arr,                0); // New item, no ID yet
                array_push($service_arr,                $item_row['service']);
                array_push($description_arr,            $item_row['description']);
                array_push($qty_arr,                    $item_row['qty']);
                array_push($rate_arr,                   $item_row['rate']);
                array_push($sub_total_arr,              $item_row['sub_total']);
                array_push($tax_arr,                    $item_row['tax']);
                array_push($tax_amount_arr,             $item_row['tax_amount']);
                array_push($total_arr,                  $item_row['total']);
            }
        }
    }
                ...
                <div class="collapse d-lg-block ms-lg-auto" id="breadcrumb_elements">
                    <div class="d-lg-flex mb-2 mb-lg-0">
                        <div class="mt-2 mb-2">
                            ...
                            <?php if (!empty($id)) { ?>
                                <a href="credit_note_overview.php?credit_note_id=<?php echo $id; ?>" class="btn btn-light btn-sm">
                                    Cancel
                                </a>
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                                <input type="text" class="form-control" placeholder="Requested Date" name="credit_note_date" id="credit_note_date" value="<?php echo $credit_note_date; ?>">
                            ...
                            <div class="row card-body">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    ?>
                                        ...
                                        <div class="mb-2">
                                            <div class="row mb-3 pb-3" id="row_<?php echo $credit_note_item; ?>">


                                                <div class="col-lg-12">
                                                    <div class="row">

                                                        <input type="hidden" name="item_id[]" id="item_id<?php echo $credit_note_item; ?>" value="<?php echo (!empty($item_id_arr[$index]) ? $item_id_arr[$index] : ''); ?>">
                                                        ...
                                                        <div class="col-lg-3">
                                                            <textarea name="description[]" id="description<?php echo $credit_note_item; ?>" rows="2" class="form-control" placeholder="Add a description to your item"><?php echo (!empty($description_arr[$index]) ? $description_arr[$index] : ''); ?></textarea>
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="qty[]" id="qty<?php echo $credit_note_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($qty_arr[$index]) ? $qty_arr[$index] : '1'); ?>" onkeyup="calculateItemAmount('<?php echo $credit_note_item; ?>');" onchange=" calculateItemAmount('<?php echo $credit_note_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="rate[]" id="rate<?php echo $credit_note_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($rate_arr[$index]) ? $rate_arr[$index] : '0'); ?>" onkeyup="calculateItemAmount('<?php echo $credit_note_item; ?>');" onchange=" calculateItemAmount('<?php echo $credit_note_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="sub_total[]" id="sub_total<?php echo $credit_note_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" value="<?php echo (!empty($sub_total_arr[$index]) ? $sub_total_arr[$index] : '0'); ?>"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <select name="tax[]" id="tax<?php echo $credit_note_item; ?>" class="form-select" onchange="calculateItemAmount(<?php echo $credit_note_item; ?>, this.value); ">
                                                                <?php
                                                                // -----------------------
                                                                for ($i = 0; $i <= 100; $i++) {
                                                                    // -----------------------
                                                            ...
                                                            <div class="text-center mt-1">
                                                                <span class="badge bg-light text-black" style="font-weight: normal;" id="div_tax_amount<?php echo $credit_note_item; ?>" style="display: <?php if (!empty($tax_arr[$index])) { ?> block <?php } else { ?> none <?php } ?>;">
                                                                    <span id="span_tax_amount<?php echo $credit_note_item; ?>">
                                                                        <?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>
                                                                    </span>
                                                                </span>
                                                            </div>

                                                            <input type="hidden" name="tax_amount[]" id="tax_amount<?php echo $credit_note_item; ?>" class="form-control" placeholder="0" value="<?php echo (!empty($tax_amount_arr[$index]) ? $tax_amount_arr[$index] : '0'); ?>">
                                                            <!-- <div class="form-text bg-light border border-top-0 rounded-bottom text-end px-2 py-1 mt-0">15,584</div> -->
                                                        </div>


                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="total[]" id="total<?php echo $credit_note_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" placeholder="0" value="<?php echo (!empty($total_arr[$index]) ? $total_arr[$index] : ''); ?>" onchange="calculateGrand(<?php echo $credit_note_item; ?>);" onkeyup="calculateGrand(<?php echo $credit_note_item; ?>);"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-2 mt-1">
                                                            <?php if ($credit_note_item > 1) { ?>
                                                                <a href="#" onclick="calculateItemAmount('<?php echo $credit_note_item; ?>'); clear_row(<?php echo $credit_note_item; ?>)"><span class="badge bg-warning"> <i class="ph-x"></i> </span></a>
                                                            <?php } ?>
                                                        </div>
                                ...
                                <div class="">
                                    <span id="span_add_item_row<?php echo $credit_note_item; ?>"><a href="#" onclick="add_item_row(); "><span class="badge bg-primary"> Add New Row </a></span></span>
                                </div>
