...
if (($action == "update_$module" || $action == "add_$module") && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // -- Recurring
    $profile_name               = e_s__($_POST['profile_name']);
    $frequency                  = e_s__($_POST['frequency']);
...
if ($action == "update_$module" && !empty($id)) {
    ...
    if (empty($customer_id) || $customer_id == 'Please select') {
    ...
    } else if (empty($profile_name)) {
        $error_message = 'Please enter Profile Name.';
    } else {
        ...
        $start_date         = processDateDtoY($start_date);
        $end_date           = (empty($end_date) ? '1970-01-01' : processDateDtoY($end_date));
        ...
        if ($update_row) {
            ...
            if ($updated_row == 0 && $inserted_row == 0) {
                ...
                if ($save_and_send == 1) {
                    header("Location:send_invoice.php?id=$id");
                } else {
                    header("Location:listing_recurring_$module.php?success_message=$success_message");
                }
            }
        } else {
            ...
            for ($invoice_item = 1; $invoice_item <= $total_rows; $invoice_item++) {
                ...
                if (!empty($item_service)) {
                    ...
                    if ($inserted_row == 0) {
                        ...
                        $insert_row = $mysqli->query("INSERT INTO `$tbl_name`(recurring, invoice_no, customer_id, invoice_status, invoice_date, expiry_date, reference_no, warehouse_id, profile_name, frequency, start_date, end_date, expected_shipment_date, payment_term, shipment_type, sales_person, job_reference_no, master_awb_no, shipper, consignee, origin, destination, no_of_packs, gross_weight, chargeable_weight, volume, terms_and_conditions, grand_subtotal, grand_discount_type, grand_discount_type_value, grand_discount_amount, grand_after_discount, customer_notes, grand_tax, grand_total, is_active) VALUES ('1', '" . $invoice_no . "', '" . $customer_id . "', '" . $invoice_status . "',  '" . $invoice_date . "', '" . $expiry_date . "', '" . $reference_no . "', '" . $warehouse_id . "', '" . $profile_name . "', '" . $frequency . "', '" . $start_date . "', '" . $end_date . "', '" . $expected_shipment_date . "', '" . $payment_term . "', '" . $shipment_type . "', '" . $sales_person . "', '" . $job_reference_no . "', '" . $master_awb_no . "', '" . $shipper . "', '" . $consignee . "', '" . $origin . "', '" . $destination . "', '" . $no_of_packs . "', '" . $gross_weight . "', '" . $chargeable_weight . "', '" . $volume . "', '" . $terms_and_conditions . "',   '" . $grand_subtotal . "',  '" . $grand_discount_type . "',  '" . $grand_discount_type_value . "',  '" . $grand_discount_amount . "',  '" . $grand_after_discount . "',   '" . $customer_notes . "',  '" . $grand_tax . "', '" . $grand_total . "', '" . $publish . "'); ");
...
// Handle source_invoice_id for creating recurring from existing invoice
$source_invoice_id = isset($_REQUEST['source_invoice_id']) ? (int)$_REQUEST['source_invoice_id'] : 0;

if ($source_invoice_id > 0 && empty($id) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    // Pre-populate from source invoice
    $source_result = $mysqli->query("SELECT * FROM `" . tbl_invoices . "` WHERE id=$source_invoice_id");
    if ($source_result && $source_row = $source_result->fetch_array()) {
        
        $customer_id            = s__($source_row['customer_id']);
        $invoice_date           = date('d-m-Y', time());
        $expiry_date            = '';
        $reference_no           = s__($source_row['reference_no']);
        $warehouse_id           = s__($source_row['warehouse_id']);

        // Recurring fields - empty for new
        $profile_name           = 'Recurring - ' . getTableAttr('display_name', tbl_customers, $customer_id);
        $frequency              = 'monthly';
        $start_date             = date('d-m-Y', time());
        $end_date               = '';

        $expected_shipment_date = s__($source_row['expected_shipment_date']);
        $payment_term           = getTableAttr('payment_term', tbl_customers, $customer_id);

        $shipment_type          = s__($source_row['shipment_type']);
        $sales_person           = s__($source_row['sales_person']);
        $job_reference_no       = s__($source_row['job_reference_no']);
        $master_awb_no          = s__($source_row['master_awb_no']);
        $shipper                = s__($source_row['shipper']);
        $consignee              = s__($source_row['consignee']);
        $origin                 = s__($source_row['origin']);
        $destination            = s__($source_row['destination']);
        $no_of_packs            = s__($source_row['no_of_packs']);
        $gross_weight           = s__($source_row['gross_weight']);
        $chargeable_weight      = s__($source_row['chargeable_weight']);
        $volume                 = s__($source_row['volume']);

        $customer_notes         = s__($source_row['customer_notes']);
        $terms_and_conditions   = s__($source_row['terms_and_conditions']);

        $grand_subtotal             = s__($source_row['grand_subtotal']);
        $grand_discount_type        = s__($source_row['grand_discount_type']);
        $grand_discount_type_value  = s__($source_row['grand_discount_type_value']);
        $grand_discount_amount      = s__($source_row['grand_discount_amount']);
        $grand_after_discount       = s__($source_row['grand_after_discount']);
        $grand_tax                  = s__($source_row['grand_tax']);
        $grand_total                = s__($source_row['grand_total']);

        $publish                    = 1;

        $expected_shipment_date = ($expected_shipment_date == '1970-01-01' ? '' : processDateYtoD($expected_shipment_date));

        // Get invoice items
        $result_source_items = $mysqli->query("SELECT * FROM `" . tbl_invoice_items . "` WHERE invoice_id=$source_invoice_id");
        $total_rows = $result_source_items->num_rows;

        if ($total_rows > 0) {
            while ($row_source_items = $result_source_items->fetch_array()) {
                array_push($item_id_arr,        0);
                array_push($service_arr,        $row_source_items['service']);
                array_push($description_arr,    $row_source_items['description']);
                array_push($qty_arr,            $row_source_items['qty']);
                array_push($rate_arr,           $row_source_items['rate']);
                array_push($sub_total_arr,      $row_source_items['sub_total']);
                array_push($tax_arr,            $row_source_items['tax']);
                array_push($tax_amount_arr,     $row_source_items['tax_amount']);
                array_push($total_arr,          $row_source_items['total']);
            }
        }
...
$created_by = getTableAttr('created_by', tbl_invoices, $id);
...
if (
    ...
    // -- Recurring
    $profile_name           = e_s__($row['profile_name']);
    $frequency              = e_s__($row['frequency']);
    $start_date             = e_s__($row['start_date']);
    $end_date               = e_s__($row['end_date']);
...
<div class="content-wrapper">
        ...
        <div class="page-header-content border-top py-2 px-3 carriers-page-header-content">
            ...
            <div class="my-1 d-inline-flex align-items-center me-2">
                <div class="form-check form-check-inline form-switch mb-0">
                    <input type="checkbox" class="form-check-input form-check-input-success" name="publish" id="publish" <?php if ($publish == '1') { ?>checked="checked" <?php } ?> form="frminvoices">
    ...
    <div class="content-inner">
        <div class="content">
            ...
            <form class="steps-basic clearfix" method="post" id="frmrecurring<?php echo $module; ?>" name="frmrecurring<?php echo $module; ?>" action="recurring_<?php echo $module; ?>.php" enctype="multipart/form-data">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <select name="warehouse_id" id="warehouse_id" class="form-select">
                                                <!-- <option value='0'>Please select</option> -->
                                                <?php
                                                $result = $mysqli->query("SELECT * FROM `" . tbl_warehouses  . "` WHERE is_active=1");
                                                while ($rows = $result->fetch_array()) {
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <select class="form-select" name="payment_term" id="payment_term">
                                                ...
                                                <?php
                                                // -------------------------------------------------------------------------------------------------
                                                $result_payment_terms = $mysqli->query("SELECT * FROM `" . tbl_payment_terms  . "` WHERE is_active=1 ORDER BY id ASC");
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label"><span class="text-danger">Profile Name:*</span></label>
                                        <div class="col-lg-9">
                                            <input type="text" class="form-control" name="profile_name" id="profile_name" value="<?php echo $profile_name; ?>">
                                        </div>
                                    </div>

                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label"><span class="text-danger">Repeat Every:*</span></label>
                                        <div class="col-lg-9">
                                            <select name="frequency" id="frequency" class="form-control">
                                                <!-- <option value='0'>Please select</option> -->

                                                <option value="week" <?php if ($action == "edit_$module" && $frequency == 'week') { ?>selected <?php } else if ($frequency == 'week') { ?>selected <?php } ?>>Week</option>
                                                <option value="2_weeks" <?php if ($action == "edit_$module" && $frequency == '2_weeks') { ?>selected <?php } else if ($frequency == '2_weeks') { ?>selected <?php } ?>>2 Weeks</option>
                                                <option value="month" <?php if ($action == "edit_$module" && $frequency == 'month') { ?>selected <?php } else if ($frequency == 'month') { ?>selected <?php } ?>>Month</option>
                                                <option value="6_months" <?php if ($action == "edit_$module" && $frequency == '6_months') { ?>selected <?php } else if ($frequency == '6_months') { ?>selected <?php } ?>>6 Months</option>
                                                <option value="year" <?php if ($action == "edit_$module" && $frequency == 'year') { ?>selected <?php } else if ($frequency == 'year') { ?>selected <?php } ?>>Year</option>
                                                <!-- <option value="week" <?php if ($action == "edit_$module" && $rows['id'] == $frequency) { ?>selected <?php } else if ($rows['id'] == $frequency) { ?>selected <?php } ?>>Custom</option> -->
                                            </select>
                                        </div>
                                    </div>
                                    <!-- 
                                    ...
                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label">Start On:</label>
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                    ...
                                    <div class="row mb-2">
                                        <label class="col-lg-3 col-form-label">Ends On:</label>
                                        <div class="col-lg-9">
                                            <div class="form-control-feedback form-control-feedback-start">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-lg-9">
                                            <select required class="form-select" name="origin" id="origin">
                                                ...
                                                <?php
                                                // -------------------------------------------------------------------------------------------------
                                                $result = $mysqli->query("SELECT * FROM `" . $tbl_prefix . "geo_countries` WHERE is_active=1 ORDER BY country_name");
                                                while ($rows = $result->fetch_array()) {
                                                    // -------------------------------------------------------------------------------------------------
                                                ?>
                                                    <option value="<?php echo $rows['id']; ?>" <?php if ($action == "edit_$module" && $rows['id'] == $origin) { ?>selected <?php } else if ($rows['id'] == $origin) { ?>selected <?php } ?>>
                                                        <?php echo $rows['alpha3_code']; ?> - <?php echo $rows['country_name']; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-lg-9">
                                            <select required class="form-select" name="destination" id="destination">
                                                ...
                                                while ($rows = $result->fetch_array()) {
                                                    // -------------------------------------------------------------------------------------------------
                                                ?>
                                                    <option value="<?php echo $rows['id']; ?>" <?php if ($action == "edit_$module" && $rows['id'] == $destination) { ?>selected <?php } else if ($rows['id'] == $destination) { ?>selected <?php } ?>>
                                                        <?php echo $rows['alpha3_code']; ?> - <?php echo $rows['country_name']; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                    ...
                    <div class="col-xl-12">
                        ...
                        <div class="card">
                            ...
                            <div class="row card-body">
                                            ...
                                            <div class="row mb-3 pb-3" id="row_<?php echo $invoice_item; ?>">
                                                ...
                                                <div class="col-lg-12">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-lg-2">
                                                            <select class="form-select" name="service[]" id="service<?php echo $invoice_item; ?>" onchange="ajax_populate_item_rate(this.value, <?php echo $invoice_item; ?>); ">
                                                                <option value="0">Please select</option>
                                                                <?php
                                                        ...
                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="qty[]" id="qty<?php echo $invoice_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($qty_arr[$index]) ? $qty_arr[$index] : '1'); ?>" onkeyup="calculateItemAmount('<?php echo $invoice_item; ?>');" onchange=" calculateItemAmount('<?php echo $invoice_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>

                                                        <div class="col-lg-1">
                                                            <input type="number" step="1" name="rate[]" id="rate<?php echo $invoice_item; ?>" min="0" class="form-control text-center" value="<?php echo (!empty($rate_arr[$index]) ? $rate_arr[$index] : '0'); ?>" onkeyup="calculateItemAmount('<?php echo $invoice_item; ?>');" onchange=" calculateItemAmount('<?php echo $invoice_item; ?>');"> <!--  step="0.1" value="0.0" -->
                                                        </div>
                                                        ...
                                                        <div class="col-lg-1">
                                                            <select name="tax[]" id="tax<?php echo $invoice_item; ?>" class="form-select" onchange="calculateItemAmount(<?php echo $invoice_item; ?>, this.value); ">
                                                                <?php
                                                                // -----------------------
                                                                for ($i = 0; $i <= 100; $i++) {
                                                                    // -----------------------
                                                        ...
                                                        <div class="col-lg-1">
                                                            <input readonly type="number" name="total[]" id="total<?php echo $invoice_item; ?>" min="0" class="form-control bg-light bg-opacity-75 text-end" placeholder="0" value="<?php echo (!empty($total_arr[$index]) ? $total_arr[$index] : ''); ?>" onchange="calculateGrand(<?php echo $invoice_item; ?>);" onkeyup="calculateGrand(<?php echo $invoice_item; ?>);"> <!--  oninput="this.value = Math.abs(this.value)" -->
                                                        </div>

                                                        <div class="col-lg-2 mt-1">
                                                            <?php if ($invoice_item > 1) { ?>
                                                                <a href="#" onclick="calculateItemAmount('<?php echo $invoice_item; ?>'); clear_row(<?php echo $invoice_item; ?>)"><span class="badge bg-warning"> <i class="ph-x"></i> </span></a>
                                                            <?php } ?>
                                                        </div>
                                ...
                                <div class="">
                                    <span id="span_add_item_row<?php echo $invoice_item; ?>"><a href="#" onclick="add_item_row(); "><span class="badge bg-primary"> Add New Row </a></span></span>
                                </div>
