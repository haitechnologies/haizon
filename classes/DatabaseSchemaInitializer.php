 ...
 * This file should be included at application load time.
 * 
 * Tables managed:
 * - erp_inquiries (contact form)
 * - erp_email_queue (email delivery)
 * - erp_rate_limit_attempts (rate limiting)
 * 
...
class DatabaseSchemaInitializer
{
     ...
     * @return string
     */
    private static function tableName($constantName, $fallback)
    {
        if (class_exists('DB') && defined('DB::' . $constantName)) {
            return constant('DB::' . $constantName);
        }
        return $fallback;
    }
    ...
    public static function init($conn)
    {
        ...
        self::ensureCompaniesLicenseNumberColumn($conn);
     ...
     */
    private static function createInquiriesTable($conn)
    {
     ...
     */
    private static function createRateLimitTable($conn)
    {
     ...
     */
    private static function createDisposableEmailDomainsTable($conn)
    {
     ...
     */
    private static function createBackendErrorLogsTable($conn)
    {
     ...
     */
    private static function createBackendLogCoverageTable($conn)
    {
    ...
    /**
     * Ensure companies table has license_number for add-business compatibility.
     */
    private static function ensureCompaniesLicenseNumberColumn($conn)
    {
        $tableName = self::tableName('COMPANIES', 'erp_companies');
        $columnName = 'license_number';

        // Check table exists before attempting to alter it
        $tableCheckSql = "
            SELECT 1
        ...
        if (!$tableStmt) {
            error_log("ERROR preparing companies table existence check: " . $conn->error);
            return;
        }
        $tableStmt->bind_param('s', $tableName);
        $tableStmt->execute();
        $tableStmt->store_result();
        $tableExists = $tableStmt->num_rows > 0;
        $tableStmt->close();
        ...
        if (!$existsStmt) {
            error_log("ERROR preparing companies license_number check: " . $conn->error);
            return;
        }

        $existsStmt->bind_param('ss', $tableName, $columnName);
        if (!$existsStmt->execute()) {
            error_log("ERROR executing companies license_number check: " . $existsStmt->error);
            $existsStmt->close();
            return;
        }
        ...
        $alterSql = "ALTER TABLE `{$tableName}` ADD COLUMN `{$columnName}` VARCHAR(100) NULL";
        if (!$conn->query($alterSql)) {
            error_log("ERROR adding license_number to {$tableName}: " . $conn->error);
        }
    }
     ...
     */
    private static function createInquiryRepliesTable($conn)
    {
        ...
        if (!$conn->query($sql)) {
            error_log("ERROR creating {$inquiryRepliesTable} table: " . $conn->error);
        }
        ...
        if ($check) {
            ...
            $check->store_result();
            if ($check->num_rows === 0) {
                $conn->query("ALTER TABLE `{$inquiryRepliesTable}`
     ...
     */
    private static function ensureUsersMfaColumns($conn)
    {
