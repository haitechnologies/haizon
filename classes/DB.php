 ...
 * 
 * Benefits:
 * - Zero performance overhead (no DB query needed)
 ...
 * 
 * Usage:
 * Instead of: tbl_users
 * Use: DB::USERS
 * 
 * Example:
 * $result = $mysqli->query("SELECT * FROM `" . DB::USERS . "` WHERE id = 1");
 */
class DB {
    ...
    public const EMAIL_CAMPAIGNS = self::PREFIX . 'email_campaigns';

    /** Email templates library */
    public const EMAIL_TEMPLATES = self::PREFIX . 'email_templates';

    /** Email campaign targets (segments) */
    public const EMAIL_TARGETS = self::PREFIX . 'email_targets';
    ...
    /** Email unsubscribes */
    public const EMAIL_UNSUBSCRIBES = self::PREFIX . 'email_unsubscribes';

    /** Email bounces */
    public const EMAIL_BOUNCES = self::PREFIX . 'email_bounces';

    /** Email tracking events */
    public const EMAIL_EVENTS = self::PREFIX . 'email_events';

    /** Campaign send aggregates */
    public const EMAIL_SENDS = self::PREFIX . 'email_sends';
    ...
    /** Customer comments and notes — merged into erp_entity_notes */
    // CUSTOMER_COMMENTS removed

    /** Customer document attachments — table dropped */
    // CUSTOMER_ATTACHMENTS removed

    /** Customer documents library — table dropped */
    // CUSTOMER_DOCUMENTS removed

    /** Customer activity logs — merged into erp_entity_logs */
    // CUSTOMER_LOGS removed
    ...
    /** Public-facing sponsored ads for text/image software campaigns */
    ...
    /** HS Code descriptions (multilingual) */
    ...
    // ================================
    // COMPANY VERIFICATION & GROWTH
    // ================================
    
    
    // ================================
    // EMAIL AUTOMATION (GROWTH)
    // ================================
    
    /** Email automation rules */
    public const EMAIL_AUTOMATION_RULES = self::PREFIX . 'email_automation_rules';
    
    /** Email automation queue */
    public const EMAIL_AUTOMATION_QUEUE = self::PREFIX . 'email_automation_queue';
    ...
    /** IP to Country geolocation database */
    ...
    /** Search queries log and analytics (Unified) */
    ...
    // ================================
    // FRONTEND USER TABLES
    // ================================
    
    /** Frontend public user accounts */
    public const FRONTEND_USERS = self::PREFIX . 'frontend_users';
    
    /** Frontend user saved companies (favorites) */
    public const FRONTEND_USER_FAVORITES = self::PREFIX . 'frontend_user_favorites';
    

    /** @deprecated Removed. Use DB::SEARCHES (unified) instead. */
    // public const FRONTEND_USER_SEARCHES = self::PREFIX . 'frontend_user_searches';
    // public const FRONTEND_SEARCH_HISTORY = self::PREFIX . 'frontend_search_history';
    // public const FRONTEND_SEARCH_PRESETS = self::PREFIX . 'frontend_search_presets';
    ...
    /** Lead notes — merged into erp_entity_notes */
    // LEAD_NOTES removed

    /** Lead attachments */
    public const LEAD_ATTACHMENTS = self::PREFIX . 'lead_attachments';

    /** Lead activity logs — merged into erp_entity_logs */
    // LEAD_LOGS removed
     ...
     * 
        * @example DB::table('users') returns 'erp_users'
     */
     ...
     * Useful for migrations, backups, data exports
     * 
     ...
     */
    public static function hasTable($constantName) {
        $reflection = new ReflectionClass(__CLASS__);
     ...
     */
    public static function getConstantName($tableName) {
        $reflection = new ReflectionClass(__CLASS__);
        $constants = $reflection->getConstants();
        
        $key = array_search($tableName, $constants);
