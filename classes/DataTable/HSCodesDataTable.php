...
class HSCodesDataTable extends BaseDataTable {
    ...
    protected function buildSearchClause($requestData) {
        ...
        if ($this->hasTextTable()) {
            return "AND (h.code LIKE '%{$searchValue}%' OR h.old_code LIKE '%{$searchValue}%' 
                    OR te.long_desc LIKE '%{$searchValue}%' OR te.short_desc LIKE '%{$searchValue}%'
                    OR ta.long_desc LIKE '%{$searchValue}%' OR ta.short_desc LIKE '%{$searchValue}%')";
        }
    ...
    private function tableExists(string $table): bool {
        try {
            ...
            error_log('[HSCodesDataTable] Error checking table existence: ' . $e->getMessage());
            return false;
        }
    }
}
