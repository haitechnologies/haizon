...
class EmailProvidersDataTable extends BaseDataTable {
    ...
    protected function formatRow($row, $requestData = []) {
        ...
        if ($mysqli instanceof mysqli) {
            $totalProvidersResult = $mysqli->query("SELECT COUNT(*) as cnt FROM `" . DB::EMAIL_PROVIDERS . "`");
            $totalProviders = $totalProvidersResult ? ($totalProvidersResult->fetch_assoc()['cnt'] ?? 0) : 0;
        }
