<?php
/**
 * SubcategoriesDataTable Handler
 */
...
class SubcategoriesDataTable extends BaseDataTable {
    ...
    protected function prepareRelatedData(array $rows, array $requestData = []): void
    {
        ...
        if ($this->tableExists(DB::CATEGORY_ITEMS)) {
            ...
            if ($result) {
                while ($row = $result->fetch_assoc()) {
                    $this->relatedDataCache['items'][(int)$row['subcategory_id']] = (int)$row['cnt'];
                }
            }
        }
        ...
        if ($this->tableExists(DB::COMPANIES)) {
            ...
            if ($result) {
                while ($row = $result->fetch_assoc()) {
                    $this->relatedDataCache['companies'][(int)$row['primary_subcategory_id']] = (int)$row['cnt'];
                }
            }
        }
    }
    ...
    protected function formatRow($row, $requestData = []) {
        ...
        $parentDisplay = $parentCategory
            ? htmlspecialchars($parentCategory, ENT_QUOTES, 'UTF-8')
