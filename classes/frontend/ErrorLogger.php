...
class FrontendErrorLogger {
    ...
    private function log($level, $message, $context = [], $file = null, $line = null) {
        ...
        if (!$file || !$line) {
            ...
            foreach ($trace as $frame) {
                ...
                if (!$file) $file = $frame['file'] ?? 'unknown';
                if (!$line) $line = $frame['line'] ?? 0;
                break;
            }
        }
    ...
    private function shouldSkipSyntheticProbe($requestedUrl, $userAgent) {
        ...
        $isSyntheticAgent = (strpos($agent, 'haipulse-manualtester') !== false || strpos($agent, 'curl/') !== false);
        if (!$isSyntheticAgent) {
            return false;
        }
    ...
    public function registerErrorHandler() {
        set_error_handler(function($errno, $errstr, $errfile, $errline) {
            ...
            if (stripos($errfile, 'dashboard') !== false || 
    ...
    public function registerExceptionHandler() {
        set_exception_handler(function($exception) {
            ...
            if (stripos($exceptionFile, 'dashboard') !== false || 
                stripos($_SERVER['REQUEST_URI'] ?? '', '/dashboard/') !== false) {
